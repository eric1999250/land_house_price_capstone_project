...
function LoginModal({ t, lang, onClose, onLoginSuccess, onSwitchToRegister }) {
  ...
  async function doLogin(e) {
    ...
    try {
      const d = await (await fetch('http://127.0.0.1:5000/auth/login', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ email: un, password: pw }) })).json();
      setLoading(false);
  ...
  async function doForgot(e) {
    ...
    try {
      const d = await (await fetch('http://127.0.0.1:5000/auth/forgot-password', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ email: fpUser }) })).json();
      setFpLoad(false);
...
function RegisterModal({ t, lang, onClose, onSwitchToLogin }) {
  ...
  async function doReg(e) {
    ...
    try {
      ...
      const d = await (await fetch('http://127.0.0.1:5000/auth/register', {
        method: 'POST',
